...
export const SignIn = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.post(ApiURL+'/user/sign-in/', postdata)
        ...
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
          <div>
            ...
            <input
              ...
              required=""
            />
             {logError &&  <span className='text-red-600'>{logError.email}</span>}
          </div>
          <div>
            <label
              htmlFor="password"
            ...
            <input
              ...
              required=""
            />
             {logError &&  <span className='text-red-500'>{logError.password}</span>}
          </div>
          ...
          <button type="submit" className="w-full text-white bg-blue-700 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
            {isLoad ? <>  <svg aria-hidden="true" class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
